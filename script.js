...
let tempTasks = [];
let pendingModalAction = null; // 'new' ou 'rename'
...
// --- GESTION MODALE INPUT (Nouveau/Renommer) ---
function openGameInputModal(action) {
    pendingModalAction = action;
    const modal = document.getElementById('game-input-modal');
    const input = document.getElementById('game-name-input');
    const title = document.getElementById('input-modal-title');
    
    input.value = (action === 'rename' && gameState) ? gameState.name : "";
    title.innerText = (action === 'rename') ? "Renommer le Projet" : "Nouvelle Aventure";
    
    document.getElementById('input-modal-confirm-btn').onclick = confirmGameInput;
...
function renderSidebar() {
    ...
    GLOBAL_TAROT_DATA.forEach((card) => {
        ...
        let summaryBtn = (data && (data.summary || (data.tasks && data.tasks.length > 0))) ? `<button class="summary-btn-side" onclick="event.stopPropagation(); openValidationModal('view', ${level}); if(window.innerWidth < 900) toggleSidebar();" title="Voir le Grimoire">📜</button>` : "";
        
        if (level === gameState.currentLevel) { 
            li.classList.add('active'); li.innerHTML = `<div>${content}</div>${summaryBtn}`; 
            li.onclick = () => { if(window.innerWidth < 900) toggleSidebar(); }; 
        }
        else if (level <= gameState.unlockedLevel) { 
            li.classList.add('done'); li.innerHTML = `<div>${content} ✓</div>${summaryBtn}`; 
            li.onclick = () => { startLevel(level, true); if(window.innerWidth < 900) toggleSidebar(); }; 
        }
...
function renderCardInfo(level) {
    ...
    const concepts = createChips(card["Concepts associés"] || "", 'concept');
...
function renderModalForm(lvl, idea, resume) {
    ...
    // Modification: Utilisation de <details> pour l'historique sur mobile
    if (lvl > 1) {
        html += `<div class="modal-section"><details ${window.innerWidth > 900 ? 'open' : ''}><summary class="modal-label" style="cursor:pointer">Précédemment (Cliquez pour voir)</summary><div class="modal-text-static">`;
        let hasPrev = false;
        for (let i = 1; i < lvl; i++) {
